...
const RealList: React.FC<Props> = () => {
  ...
  const squares: Square[] = [
    {
      ...
      cuota: '1.40',
      recompensa:
        searchText !== '' ? (parseFloat(searchText) * 1.4).toFixed(2) : '...',
    ...
    },
    {
      ...
      cuota: '2.50',
      recompensa:
        searchText !== '' ? (parseFloat(searchText) * 2.5).toFixed(2) : '...',
    ...
    },
    {
      ...
      cuota: '1.42',
      recompensa:
        searchText !== '' ? (parseFloat(searchText) * 1.42).toFixed(2) : '...',
  ...
  const renderSquare = ({
    ...
    <View style={styles.square}>
      <TouchableOpacity
        ...
        style={[styles.checkbox, selectedIndex === index && styles.checked]}>
        <Image source={item.image} style={styles.checkboxImage} />
      </TouchableOpacity>
      <Text style={styles.checkboxText}>
        <Text style={styles.boldText}>{item.title}</Text>
        {item.subtitle ? (
          <Text style={styles.describirText}>{item.subtitle}</Text>
        ) : null}
  ...
  useEffect(() => {
    const fetchSaldo = async () => {
      const respose = await UsuarioService.getSaldo();
      console.log('@@@@@@@@@@@@@@ THIS IS THE RESPONSE OF USER INFO ->');
      console.log({saldO: respose});
    };
        ...
        <ScrollView style={styles.container}>
          <View style={styles.searchContainer}>
            ...
            <View style={styles.saldoDisponibleContainer}>
              <Image
                source={require('../images/recargar.png')}
                style={styles.recargarIcon}
              />
          ...
          <View style={styles.apuestasRealizadas}>
            <Text style={styles.apuestasRealizadasTitle}>
              APUESTAS REALIZADAS
            </Text>
          </View>
          ...
          <View style={styles.rectangle}>
            ...
            <Text style={styles.rectangleText}>
              Recuerda, poner primero la apuesta y después buscar la partida
              clasificatoria.{'\n'}
              {'\n'}
              No se tolerará cuentas{' '}
              <Text style={styles.greenTextReglas}>“smurf”</Text> o{' '}
              <Text style={styles.greenTextReglas}>“boosters”</Text> en el
              juego, evita que tu cuenta sea deshabilitada para siempre y juega
              con tu cuenta y medalla real.{'\n'}
              {'\n'}
              Las partidas de ranked grupal{' '}
              <Text style={styles.greenTextReglas}>NO</Text> están permitidas,
              si lo haces tu partida será identificada y serás penalizado.{'\n'}
              {'\n'}
              Una vez ejecutada la apuesta tienes{' '}
              <Text style={styles.greenTextReglas}>
                25 minutos para empezar a jugar{' '}
              </Text>
              en modo clasificatoria.{'\n'}
              {'\n'}
              No está permitido utilizar cuentas secundarias para ingresar a
              partidas repletas de novatos abusando de jugadores que recién
              empiezan. {'\n'}
              {'\n'}
            </Text>
            <Text style={styles.rectangleSubtitle}>APUESTA POR MEDALLA:</Text>
            <Text style={styles.rectangleText}>
              Hasta Guardián, monto máximo de apuesta es de 10 soles.
              {'\n'}
